...
import { lt2Presenters, type Lt2Presenter } from "../../shared/lt2Presenters.js";
...
function parseAdminIds() {
  const raw = process.env.LT2_CANCEL_ADMIN_IDS ?? process.env.ADMIN_DISCORD_IDS ?? "";
  return new Set(
    raw
      .split(",")
...
function canCancelPresenter(sessionUserId: string, presenter: Lt2Presenter) {
...
export default async function handler(
  ...
  if (!enforceFeatureEnabled(res, "LT2_PRESENTER_CANCEL_ENABLED", false)) {
    return;
  }
  ...
  const userKey = `lt2:presenter-cancel:${session.sub}`;
  ...
  console.log(`[LT2] Presenter cancelled: ${safePresenter} (${safePresenterId}) by ${safeDisplayName}`);
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[LT2] Presenter cancel webhook failed: ${response.status}`);
      return sendJson(res, 502, { error: "Failed to send cancel notice." });
    }
  } catch (error) {
    console.error(`[LT2] Presenter cancel webhook error: ${error instanceof Error ? error.message : "unknown error"}`);
    return sendJson(res, 502, { error: "Failed to send cancel notice." });
  } finally {

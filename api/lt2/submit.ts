...
export default async function handler(
  ...
  if (!enforceFeatureEnabled(res, "LT2_SUBMIT_ENABLED", true)) {
    return;
  }
  ...
  const userKey = `lt2:submit:${session.sub}`;
  ...
  if (trustedIp) {
    const ipKey = `lt2:submit-ip:${trustedIp}`;
    ...
    const globalKey = "lt2:submit-global";
  ...
  console.log(`[LT2] Submission: ${safeDisplayName} (${session.sub}) - ${safeTitle}`);
  ...
  try {
    ...
    console.error(`[LT2] Webhook request failed: ${error instanceof Error ? error.message : "unknown error"}`);
  ...
  if (!response.ok) {
    console.error(`[LT2] Failed to send webhook: ${response.status} ${response.statusText}`);
    return sendJson(res, 502, { error: "Failed to deliver to Discord." });
  }
